...
# the argument is a double number. If the argument is a double number, return the double number as-is.

def twice(number):
    digits = str(number)
    center = len(digits) // 2
    is_double = (len(digits) % 2 == 0) and (digits[:center] == digits[center:])
